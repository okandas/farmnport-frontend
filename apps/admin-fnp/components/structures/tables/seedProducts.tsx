...
export function SeedProductsTable() {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ["seed-products", { page, search }],
        queryFn: () => querySeedProducts({ p: page, search }),
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-white/10">
                    <thead className="bg-gray-50 dark:bg-white/5">
                        <tr>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Variety</th>
                    ...
                    <tbody className="bg-white dark:bg-transparent divide-y divide-gray-100 dark:divide-white/5">
                        {isLoading && (
                            <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-gray-500"><Icons.spinner className="w-5 h-5 mx-auto animate-spin" /></td></tr>
                        )}
                        {!isLoading && products.length === 0 && (
                            <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">No seed products found.</td></tr>
                        )}
                        {products.map((product: any) => (
                            <tr key={product.id} className="hover:bg-muted/50">
                                <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white capitalize">{product.name}</td>
                                <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{product.type?.replace("_", " ")}</td>
                                <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{product.variety || "—"}</td>
