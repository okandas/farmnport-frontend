...
import { queryUserViews } from "@/lib/query"
...
export default function UserViewsPage() {
  ...
  const { data } = useQuery({
    queryKey: ["user-views", pagination.pageIndex + 1],
    queryFn: () => queryUserViews(pagination.pageIndex + 1, PAGE_SIZE),
  ...
  return (
    <DashboardShell>
      <DashboardHeader
        heading="User Views"
        text="How much each user views others and how much they are viewed."
      />
      <DataTable
        columns={columns}
        data={users}
