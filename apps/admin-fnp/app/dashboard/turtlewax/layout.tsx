...
import { turtlewaxDashboardConfig } from "@/config/turtlewax-dashboard"
...
export const metadata = {
  title: "OI - Turtlewax Management",
}

interface TurtlewaxDashboardLayoutProps {
  children?: React.ReactNode
}

export default async function TurtlewaxDashboardLayout({
  children,
}: TurtlewaxDashboardLayoutProps) {
  return (
      ...
      <div className="max-w-full flex flex-1 px-8">
        <aside className="fixed top-16 hidden h-[calc(100vh-4rem)] w-[220px] flex-col border-r md:flex">
          <ScrollArea className="flex-1 px-4 py-4">
            <SidebarNavigation
              navigationGroups={turtlewaxDashboardConfig.sidebarNavigation}
            />
          </ScrollArea>
        </aside>
