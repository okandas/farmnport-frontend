...
import { chefsDashboardConfig } from "@/config/chefs-dashboard"
...
export const metadata = {
  title: "OI - Chefs Management",
}

interface ChefsDashboardLayoutProps {
  children?: React.ReactNode
}

export default async function ChefsDashboardLayout({
  children,
}: ChefsDashboardLayoutProps) {
  return (
      ...
      <div className="max-w-full flex flex-1 px-8">
        <aside className="fixed top-16 hidden h-[calc(100vh-4rem)] w-[220px] flex-col border-r md:flex">
          <ScrollArea className="flex-1 px-4 py-4">
            <SidebarNavigation
              navigationGroups={chefsDashboardConfig.sidebarNavigation}
            />
          </ScrollArea>
        </aside>
