...
export function SeedProductCard({ product, mode }: SeedProductCardProps) {
  const href = `/buy-seed-products/${product.slug}`
    ...
    <div className="bg-card border border-border rounded-lg overflow-hidden transition-all duration-200 hover:shadow-lg hover:border-primary/50 group">
        ...
        <div className="relative aspect-square bg-white">
          {product.images?.[0]?.img?.src ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-muted/30">
              <Sprout className="w-16 h-16 text-muted-foreground/20" />
            </div>
          )}
        </div>
      </Link>
      ...
      <div className="p-4 space-y-3 border-t">
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground pt-2 border-t">
          ...
          {product.type && (
            <span className="capitalize text-muted-foreground/70">· {product.type.replace("_", " ")}</span>
          )}
        </div>
