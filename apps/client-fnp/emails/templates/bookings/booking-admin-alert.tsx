      ...
      <Body style={body}>
        <Container style={container}>
          ...
          <Section style={header}>
            ...
            <Text style={brandTagline}>admin alert</Text>
          </Section>
          ...
          <Section style={content}>
            ...
            <Section style={pillWrapper}>
              <Text style={pill}>⚡ &nbsp;New {typeLabel}</Text>
            </Section>
            ...
            <Row style={detailRow}>
              <Column style={detailKey}><Text style={keyText}>Name</Text></Column>
              <Column><Text style={valText}>{customerName}</Text></Column>
            </Row>
            <Row style={detailRow}>
              <Column style={detailKey}><Text style={keyText}>Email</Text></Column>
              <Column><Text style={valText}>{customerEmail}</Text></Column>
            </Row>
            <Row style={detailRow}>
              <Column style={detailKey}><Text style={keyText}>Phone</Text></Column>
              <Column><Text style={valText}>{customerPhone}</Text></Column>
            </Row>

            <Hr style={thinDivider} />

            <Text style={sectionLabel}>Booking</Text>
            <Row style={detailRow}>
              <Column style={detailKey}><Text style={keyText}>Type</Text></Column>
              <Column><Text style={valText}>{typeLabel}</Text></Column>
            </Row>
            <Row style={detailRow}>
              <Column style={detailKey}><Text style={keyText}>Product</Text></Column>
              <Column><Text style={valText}>{productName}</Text></Column>
            </Row>

            {isPreOrder && (
              <Row style={detailRow}>
                <Column style={detailKey}><Text style={keyText}>Quantity</Text></Column>
                <Column><Text style={valText}>{quantity} {unit}</Text></Column>
              </Row>
            )}

            {!isPreOrder && goods && (
              <Row style={detailRow}>
                <Column style={detailKey}><Text style={keyText}>Goods</Text></Column>
                <Column><Text style={valText}>{goods}</Text></Column>
              </Row>
            )}

            {collectionPoint && (
              <Row style={detailRow}>
                <Column style={detailKey}><Text style={keyText}>Location</Text></Column>
                <Column><Text style={valText}>{collectionPoint}</Text></Column>
              </Row>
            )}

            {bookingDate && (
              <Row style={detailRow}>
                <Column style={detailKey}><Text style={keyText}>Date</Text></Column>
                <Column><Text style={valText}>{bookingDate}{timeSlot ? ` · ${timeSlot}` : ""}</Text></Column>
              </Row>
            )}

            <Section style={buttonWrapper}>
              <Button href={adminUrl} style={button}>Review in admin</Button>
            </Section>
...
const pill: React.CSSProperties = { display: "inline-block", fontSize: "12px", letterSpacing: "0.04em", color: "#ea580c", border: "1px solid #ea580c", borderRadius: "999px", padding: "4px 12px", margin: 0 }
...
const sectionLabel: React.CSSProperties = { fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.08em", color: "#94a3b8", margin: "20px 0 10px" }
const detailRow: React.CSSProperties = { marginBottom: "8px" }
const detailKey: React.CSSProperties = { width: "140px", verticalAlign: "top" }
