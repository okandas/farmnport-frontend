...
export function BuyEquipmentClient({ initialProducts, initialTotal, categories }: BuyEquipmentClientProps) {
    ...
    const { data: productsData, isLoading: productsLoading } = useQuery({
        queryKey: ["equipment-shop", queryState.p, queryState.brand],
        queryFn: () => queryBuyEquipmentProducts({
            p: queryState.p,
            ...
            <main className="flex-1">
                {productsLoading ? (
                    ...
                    <div className="text-center py-12">
                        <p className="text-muted-foreground">No equipment products found matching your filters.</p>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                            {products.map((product: any) => (
                                <ProductCard
                                    key={product.id}
                                    href={`/buy-equipment/${product.slug}`}
