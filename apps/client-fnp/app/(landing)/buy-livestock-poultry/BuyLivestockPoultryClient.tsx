...
interface BuyLivestockPoultryClientProps {
  initialProducts: any[]
  initialTotal: number
}

export function BuyLivestockPoultryClient({ initialProducts, initialTotal }: BuyLivestockPoultryClientProps) {
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
            {products.map((product: any) => (
              <LivestockPoultryCard key={product.id} product={product} mode="shop" />
            ))}
          </div>
          {totalPages > 1 && (
