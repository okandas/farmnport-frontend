          ...
          {feedsTotal > 0 && (
            <Section label="Animal Feed" href="/buy-feeds" count={feedsTotal}>
              {feeds.slice(0, 4).map((p) => (
                <ProductCard key={p.id} mode="buy" href={`/buy-feeds/${p.slug}`} imageSrc={p.images?.[0]?.img?.src} name={p.name} brand={p.brand?.name} meta={p.animal ? `${p.animal}${p.phase ? ` · ${p.phase}` : ""}` : undefined} productId={p.id} productType="feed" productSlug={p.slug} showPrice={p.show_price} salePrice={p.sale_price} wasPrice={p.was_price} showWasPrice={p.show_was_price} availableForSale={p.available_for_sale} stockLevel={p.stock_level} hasVariants={p.variants?.length > 0} variantPriceRange={p.variant_price_range} pickupOnly={p.pickup_location_ids?.length > 0 && !p.delivery_available && !(p.delivery_location_ids?.length > 0)} />
              ))}
            </Section>
          )}
          ...
          {plantNutritionTotal > 0 && (
            <Section label="Plant Nutrition" href="/buy-plant-nutrition" count={plantNutritionTotal}>
              {plantNutrition.slice(0, 4).map((p) => (
                <ProductCard key={p.id} mode="buy" href={`/buy-plant-nutrition/${p.slug}`} imageSrc={p.images?.[0]?.img?.src} name={p.name} brand={p.brand?.name} meta={p.plant_nutrition_category?.name} productId={p.id} productType="plant_nutrition" productSlug={p.slug} showPrice={p.show_price} salePrice={p.sale_price} wasPrice={p.was_price} showWasPrice={p.show_was_price} availableForSale={p.available_for_sale} stockLevel={p.stock_level} hasVariants={p.variants?.length > 0} variantPriceRange={p.variant_price_range} pickupOnly={p.pickup_location_ids?.length > 0 && !p.delivery_available && !(p.delivery_location_ids?.length > 0)} />
              ))}
            </Section>
          )}
          ...
          {seedsTotal > 0 && (
            <Section label="Seeds" href="/buy-seed-products" count={seedsTotal}>
              {seeds.slice(0, 4).map((p) => {
                ...
                return (
                  <ProductCard key={p.id} mode="buy" href={`/buy-seed-products/${p.slug}`} imageSrc={p.images?.[0]?.img?.src} name={p.name} brand={p.brand?.name} meta={p.variety ? `${p.variety}${p.type ? ` · ${p.type.replace("_", " ")}` : ""}` : undefined} productId={p.id} productType="seed_product" productSlug={p.slug} showPrice={p.show_price} salePrice={p.sale_price} wasPrice={p.was_price} showWasPrice={p.show_was_price} availableForSale={p.available_for_sale} stockLevel={p.stock_level} hasVariants={p.variants && p.variants.length > 0} variantPriceRange={p.variant_price_range} preorderHref={bookingEvent ? `/bookings/${bookingEvent.slug}` : undefined} />
                )
              })}
            </Section>
          )}
